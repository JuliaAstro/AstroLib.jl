...
function ismeuv(wave::T, hcol::T, he1col::T, he2col::T, fano::Bool) where {T <: AbstractFloat}
    ...
    z = sqrt(r / (1 - r))
    denom = -expm1(-2 * T(π) * z)
    tauH = (3.44e-16 * r^4 * exp(-4z * atan(1 / z)) * hcol) / denom
    r *= 4
    tauHe2 = zero(T)
    if r < 1
        z = sqrt(r / (1 - r))
        denom = -4 * expm1(-2 * T(π) * z)
        tauHe2 = (3.44e-16 * r^4 * exp(-4z * atan(1 / z)) * he2col) / denom
    end
    q = (2.81, 2.51, 2.45, 2.44)
    fano_gamma = (2.64061e-3, 6.20116e-4, 2.56061e-4, 1.320159e-4)
    ...
    if wave < 503.97
        ...
        if wave < 46
            y = @evalpoly(
                x, -2.465188e+1, 4.354679, -3.553024, 5.57304,
                -5.872938, 3.720797, -1.226919, 1.576657e-1
            )
        else
            y = @evalpoly(
                x, -2.953607e+1, 7.083061, 8.678646e-1, -1.221932,
                4.052997e-2, 1.317109e-1, -3.265795e-2,
                2.500933e-3
            )
            if fano
...
"""
ismeuv(
    wave::Real, hcol::Real, he1col::Real = hcol / 10, he2col::Real = 0,
    fano::Bool = false
) = ismeuv(
    promote(float(wave), float(hcol), float(he1col), float(he2col))..., fano
)
