...
Solve Kepler's equation in the elliptic motion regime (``0 ≤ e ≤ 1``)
...
where ``M(t) = (t - t_0)/P`` is the mean anomaly, ``E(t)`` the eccentric
...
- Find the eccentric anomaly for an orbit with eccentricity ``e = 0.7`` and
  for ``M(t) = 8π/3``.
