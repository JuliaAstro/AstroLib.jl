...
function precess_cd(
        cd::AbstractMatrix{T}, epoch1::T, epoch2::T,
        crval_old::AbstractVector{T}, crval_new::AbstractVector{T},
        FK4::Bool
    ...
    if (epoch1 == 2000 && epoch2 == 1950) || (epoch1 == 1950 && epoch2 == 2000)
        ...
        pole_ra, pole_dec = precess(pole_ra, pole_dec, epoch1, epoch2; FK4)
    end
...
"""
function precess_cd(
        cd::AbstractMatrix{<:Real}, epoch1::Real, epoch2::Real,
        crval_old::AbstractVector{<:Real}, crval_new::AbstractVector{<:Real},
        FK4::Bool = false
    )
    return precess_cd(
        float(cd), promote(float(epoch1), float(epoch2))...,
