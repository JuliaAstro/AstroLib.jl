...
function _mag2geo(lat::T, long::T, pole_lat::T, pole_long::T) where {T <: AbstractFloat}
    ...
    maglong2geolong = SMatrix{3, 3}(
        cos_pole_long, sin_pole_long, zero(T),
...
"""
mag2geo(lat::Real, long::Real, year::Real = Dates.year(Dates.now())) = _mag2geo(
    promote(
...
function mag2geo(
        lat::AbstractArray{LA}, long::AbstractArray{LO},
        year::Real = Dates.year(Dates.now())
    ) where {LA <: Real, LO <: Real}
    ...
    for i in eachindex(lat)
        geolat[i], geolong[i] = _mag2geo(
            promote(float(lat[i]), float(long[i]), polelat, polelong)...
        )
    end
    return geolat, geolong
end
