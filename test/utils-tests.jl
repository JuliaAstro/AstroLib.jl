...
@testset "adstring" begin
    @test @inferred(adstring((30.4, -1.23), truncate = true)) ==
        ...
        " 02 01 35.9  -01 13 48"
    @test @inferred(adstring(15, -0.5)) == " 01 00 00.0  -00 30 00"
...
@testset "aitoff" begin
    @test @inferred(aitoff([227.23, 130], [-8.89, -35])) ==
        (
    ...
    @test @inferred(aitoff((227.23, -8.89))) ==
...
@testset "altaz2hadec" begin
    ...
    @test @inferred(
        altaz2hadec([15, 25, 35], [25.12, 45.32, -20.3], [-23.44, 45.0, 52.5])
    ) == (
...
@testset "bprecess" begin
    ra, dec = @inferred(
        bprecess(
            [ten(13, 42, 12.74) * 15], [ten(8, 23, 17.69)],
            reshape(100 * [-15 * 0.0257, -0.09], 2, 1)
        )
    )
    ...
    ra, dec = @inferred(
        bprecess(
            [57], [23], reshape([9, 86], 2, 1), parallax = [1], radvel = [4]
        )
    )
...
@testset "co_aberration" begin
    d_ra, d_dec = @inferred(
        co_aberration(
            jdcnv(1987, 4, 10, 0), ten(2, 46, 11.331) * 15,
            ten(49, 20, 54.54), 1
        )
    )
    ...
    ao, bo = @inferred(
        co_aberration(
            [57555.0, -6.44311e5], [302.282, 69.5667], [37.1519, 20.6847]
        )
    )
...
@testset "co_nutate" begin
    an, bn, cn, dn, en = @inferred(
        co_nutate(
            [jdcnv(2028, 11, 13, 4, 56), jdcnv(2013, 4, 16)],
            [10, 160], [80, 30]
        )
    )
    ...
    ra_out, dec_out, eps_out, d_psi_out, d_eps_out = @inferred(
        co_nutate(2.451545e6, 325, 0)
    )
...
@testset "deredd" begin
    by0, m0, c0, ub0 = @inferred(
        deredd([0.5, -0.5], [0.2, 0.5], [1, 1], [1, 1], [0.1, 0.3])
    )
    @test by0 ≈ [-0.3, 0.5]
    @test m0 ≈ [1.165, 1.0]
    @test c0 ≈ [0.905, 1.0]
...
@testset "eq2hor" begin
    alt_o, az_o, ha_o = eq2hor(
        259.20238631600944, 49.674907472176095,
    ...
    alt_o, az_o, ha_o = eq2hor(
        142.2933457820434, -34.218006262991786, 2.0e6, 54.435, -34.78,
...
@testset "euler" begin
    ...
    elong, elat = @inferred(
        euler(
            3.141592653589793, 0.6143838917832061, 3,
    ...
    ra, dec = @inferred(
        euler(
            (2.8679433080257506, 0.557258307291505), 4,
            FK4 = true, radians = true
        )
    )
...
@testset "gal_uvw" begin
    u, v, w = @inferred(
        gal_uvw(
            [ten(1, 9, 42.3) * 15], [ten(61, 32, 49.5)], [627.89], [77.84],
            [-321.4], [1.0e3 / 129], lsr = true
        )
    )
...
@testset "geodetic2geo" begin
    ...
    @test lat ≈ 67.2 atol = 1.0e-8
    @test long == 13.4
    @test alt ≈ 1.2 atol = 1.0e-9
end
...
@testset "helio_rv" begin
    @test @inferred(
        helio_rv(
            helio_jd(
                juldate(94, 10, 25, 17, 30), ten(04, 38, 16) * 15,
                ten(20, 41, 05)
            ), 46487.5303, 2.0563056, -6, 59.3
        )
    ) ≈
        -62.965570109145034
...
@testset "hor2eq" begin
    ...
    ra_o, dec_o, ha_o = hor2eq(
        1.345, 359.43, 2.0e6, 54.435, -34.78, 1000.34, ws = true,
    ...
    @test ha_o ≈ 359.3108663499664
    ra_o, dec_o, ha_o = hor2eq(
        ten(37, 54, 41), ten(264, 55, 06), 2466879.7083333,
        "kpno", pressure = 711, temperature = 273
    )
    @test ra_o ≈ 3.3222617779538037
    @test dec_o ≈ 15.190516725395284
    @test ha_o ≈ 54.61193186104758
    ra_o, dec_o, ha_o = @inferred(
        AstroLib._hor2eq(
            43.6879, 56.684, Float64(AstroLib.J2000),
    ...
    ra_o, dec_o = hor2eq(eq2hor(45, 60, 2.0e6, "pbo")[1:2]..., 2.0e6, "pbo")
...
@testset "ismeuv" begin
    @test @inferred(ismeuv(304, 1.0e20)) ≈ 58.30508020244554
    @test ismeuv.([50, 75, 343], 1.0e18) ≈
        [0.004486567212077276, 0.01601219540569213, 0.7811526665834057]
    @test ismeuv.([96, 41, 233], 1.0e18, 1.0e17, 1.0e17) ≈
        [0.04657033979688484, 0.0035293162863089807, 0.30662192095758833]
    @test ismeuv.([96.6, 41.056, 233.19], 1.0e18, 1.0e17, 1.0e17, true) ≈
        [0.04733922036264192, 0.003543553203857853, 0.3103333860793942]
    @test ismeuv.([480, 910], 1.0e19, 5.0e17, 5.0e17) ≈
        [14.273937721143753, 62.68379602891728]
    @test @inferred(ismeuv(4500, 1.0e18)) ≈ 0
end
...
@testset "jprecess" begin
    ra, dec = @inferred(
        jprecess(
            [ten(13, 39, 44.526) * 15], [ten(8, 38, 28.63)],
    ...
    ra, dec = @inferred(
        jprecess(
            [57], [23], reshape([9, 86], 2, 1), parallax = [1],
            radvel = [4]
        )
    )
...
@testset "juldate" begin
    ...
    # Calendar.
    @test (
        dt = DateTime(2016, 1, 1, 20, 45, 33, 457);
...
@testset "mag2flux" begin
    ...
    @test flux2mag(mag2flux(15, ABwave = 12.0), ABwave = 12) ≈ 15.0 # Inferred Type Error
...
@testset "paczynski" begin
    @test @inferred(paczynski(-1.0e-10)) ≈ -1.0e10
    @test @inferred(paczynski(1.0e-1)) ≈ 10.037461005722337
    @test @inferred(paczynski(-1)) ≈ -1.3416407864998738
    @test @inferred(paczynski(10)) ≈ 1.0001922892047386
    @test @inferred(paczynski(-1.0e10)) ≈ -1
end
...
@testset "planet_coords" begin
    @test_throws DomainError @inferred(
        planet_coords(DateTime(2013, 07, 22, 03, 19, 06), 0)
    )
...
@testset "posang" begin
    @test @inferred(
        posang(
            1, ten(13, 25, 13.5), ten(54, 59, 17),
            ten(13, 23, 55.5), ten(54, 55, 31)
        )
    ) ≈ -108.46011246802047
...
@testset "precess" begin
    ...
    ra2, dec2 = @inferred(
        precess(
            [ten(21, 59, 33.053) * 15], [ten(-56, 59, 33.053)],
            1950, 1975, FK4 = true
        )
    )
...
@testset "precess_cd" begin
    ...
    @test @inferred(
        precess_cd(
            [12.45 56.7; 66 89], 2000, 1985, [67.4589455, 0.345345],
    ...
    @test @inferred(
        precess_cd(
            [30.0 28.967; 60.45 90.65], 2000, 1975, [13, 10.658],
            [35.54, 67], true
        )
    ) ≈ [
...
@testset "rhotheta" begin
    ρ, θ = @inferred(
        rhotheta(41.623, 1934.008, 0.2763, 0.907, 59.025, 23.717, 219.907, 1980)
    )
...
@testset "uvbybeta" begin
    ...
    te_o, mv_o, eby_o, delm_o, radius_o = @inferred uvbybeta(
        0.18547, -0.04356, 0.2576, 6, 2.72001
    )
    @test te_o ≈ 7308.315628104092
    ...
    te_o, mv_o, eby_o, delm_o, radius_o = @inferred uvbybeta(
        0.18547, -0.04356, 0.2576, 6, 2.8201
    )
